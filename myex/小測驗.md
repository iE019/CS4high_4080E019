...
add = str(a)+"/"+str(b)+"="+str(c)  #指定，整數1轉換成字串/整數2也轉換成字串=整數0.5轉換成的字串，給變數add
...
```
# 10-5
num = int(input ("Enter a number to find the square of: "))  #指定，輸入一個數字來求的平方，給變數num
user_input = input("Enter a integer to find the square of: ") #指定，輸入一個整數來求的平方，給變數ser_input
...
# 13-5
num_a = int(input("pick a number:")) #輸入整數給變數num_a
num_b = int(input("pick a number:")) #輸入整數給變數num_b
if num_a < 0 and num_b < 0: #如果兩次所輸入的整數都<0
    print("both negative") #則列印出字串both negative，否則，不會列印出字串
