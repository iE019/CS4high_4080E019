...
答:
可靠的reliable
由於網路擁塞和錯誤，封包可能在傳輸過程中遺失。通過偵錯碼，傳輸協定可以檢查資料是否損壞，
並通過向傳送者傳ACK(確認信息，是在電腦網路中通訊協定的一部份，是裝置或是行程發出的訊息，回覆已收到資料)
或NACK(在通訊時表示否定應答，一般代表通訊連路已建立，但有一些問題)訊息確認正確接收。
自動重發請求方案可用於重新傳輸遺失或損壞的資料。
例:TCP是一種可靠單播協定。
